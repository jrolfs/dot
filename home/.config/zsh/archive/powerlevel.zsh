echo "powelevel" > ~/Desktop/powerlevel.txt

# Generated by Powerlevel10k configuration wizard on 2019-08-07 at 10:09 PDT.
# Based on romkatv/powerlevel10k/config/p10k-lean.zsh, checksum 52466.
...
() {
  ...
  typeset -g POWERLEVEL9K_PROMPT_CHAR_OK_{VIINS,VICMD,VIVIS}_FOREGROUND=7
  ...
  vcs+='${${VCS_STATUS_LOCAL_BRANCH:+%14F'${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}'${VCS_STATUS_LOCAL_BRANCH//\%/%%}}'
  vcs+=':-%f@%14F${VCS_STATUS_COMMIT[1,8]}}'
  # ':master' if the tracking branch name differs from local branch.
  vcs+='${${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH}:+%f:%14F${VCS_STATUS_REMOTE_BRANCH//\%/%%}}'
  # '#tag' if on a tag.
  vcs+='${VCS_STATUS_TAG:+%f#%14F${VCS_STATUS_TAG//\%/%%}}'
  ...
  vcs+='${${VCS_STATUS_COMMITS_AHEAD:#0}:+${${(M)VCS_STATUS_COMMITS_BEHIND:#0}:+ }%14F⇡${VCS_STATUS_COMMITS_AHEAD}}'
  # *42 if have stashes.
  vcs+='${${VCS_STATUS_STASHES:#0}:+ %14F*${VCS_STATUS_STASHES}}'
  # 'merge' if the repo is in an unusual state.
  vcs+='${VCS_STATUS_ACTION:+ %9F${VCS_STATUS_ACTION//\%/%%}}'
